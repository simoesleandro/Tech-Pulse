...
from app.services.obsidian_agent import agente_obsidian, fallback_obsidian_body
...
def test_fallback_obsidian_body_uses_callouts():
    body = fallback_obsidian_body(_sample_item())
...
def test_news_item_to_markdown_includes_frontmatter():
    markdown = obsidian_service.news_item_to_markdown(_sample_item())
...
def test_build_obsidian_note_combines_frontmatter_and_body():
    body = "# Título\n\n> [!info] Teste"
    note = obsidian_service.build_obsidian_note(_sample_item(), body)
...
def test_agente_obsidian_falls_back_on_invalid_response():
    ...
    with patch(
        ...
        body = asyncio.run(agente_obsidian(item))

    assert "> [!abstract] O que é" in body


def test_export_items_filesystem_uses_agent(tmp_path, monkeypatch):
...
"""
    ...
    with patch(
        ...
        new=AsyncMock(return_value=agent_body),
    ):
